...
const defaultNavItems = [
  { id: 'hub', label: 'Home', route: '/hub', icon: '/botao-home.png', isImage: true },
  { id: 'explorar', label: 'Explorar', route: '/explorar', icon: '/lancamento-do-produto.png', isImage: true },
  { id: 'profissionais', label: 'Profissionais', route: '/profissionais', icon: '/homem-de-negocios.png', isImage: true },
]

const notifications = [
  { id: 1, icon: '💰', title: 'Novo orçamento recebido', subtitle: 'João Silva enviou um orçamento', time: 'Há 5 min' },
  { id: 2, icon: '💬', title: 'Profissional respondeu', subtitle: 'Maria Santos respondeu sua solicitação', time: 'Há 15 min' },
  { id: 3, icon: '✅', title: 'Pedido concluído', subtitle: 'Serviço de limpeza finalizado', time: 'Há 32 min' },
  { id: 4, icon: '📦', title: 'Novo pedido recebido', subtitle: 'Você tem um novo pedido de serviço', time: 'Há 1 hora' },
]
    ...
    <nav className={styles.nav}>
      <div className={styles.navInner}>
        ...
        <div className={styles.rightGroup}>
          <button className={styles.iconBtn} title="Buscar" onClick={() => navigate('/explorar')}>
          ...
          <div className={styles.notifWrapper} ref={notifRef}>
            <button className={styles.iconBtn} onClick={() => { setNotifOpen(!notifOpen); setProfileOpen(false) }} title="Notificações">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="20" height="20">
            ...
            {notifOpen && (
              <div className={styles.notifPanel}>
                <div className={styles.notifHeader}>
                  ...
                  <button className={styles.notifAction}>Marcar todas como lidas</button>
                </div>
                  ...
                  {notifications.map((n) => (
                    <div key={n.id} className={styles.notifItem}>
                      ...
                      <div className={styles.notifItemContent}>
                        ...
                        <span className={styles.notifItemSub}>{n.subtitle}</span>
                      </div>
                ...
                <div className={styles.dropdown}>
                  <div className={styles.dropdownHeader}>
                    ...
                    <div>
                      ...
                      <span className={styles.dropdownRole}>{user.role === 'prestador' ? 'Prestador' : 'Cliente'}</span>
                    </div>
                  </div>
                  <button className={styles.dropdownItem} onClick={() => { setProfileOpen(false); navigate('/perfil') }}>
                  ...
                  <button className={styles.dropdownItem} onClick={() => { setProfileOpen(false); navigate('/meus-pedidos') }}>
                    <span className={styles.dropdownItemIcon}>
                  ...
                  <div className={styles.dropdownDivider} />
                  <button className={styles.dropdownItem} onClick={() => { setProfileOpen(false); navigate('/configuracoes') }}>
                    <span className={styles.dropdownItemIcon}>
                  ...
                  <button className={styles.dropdownItem} onClick={() => { setProfileOpen(false); navigate('/ajuda') }}>
                    <span className={styles.dropdownItemIcon}>
