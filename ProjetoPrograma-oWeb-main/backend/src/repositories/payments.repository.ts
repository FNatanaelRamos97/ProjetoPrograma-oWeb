...
export class PaymentsRepository {
  ...
  async findByStripeSessionId(stripeCheckoutSessionId: string) {
    const result = await db
      .select()
      .from(payments)
  ...
  async markStripePaymentAsPaid(data: {
    stripeCheckoutSessionId: string;
    stripePaymentIntentId?: string | null;
  }) {
    const existing = await this.findByStripeSessionId(data.stripeCheckoutSessionId);
