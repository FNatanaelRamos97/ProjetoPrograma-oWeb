...
export default function Agenda() {
  ...
  const [providerId, setProviderId] = useState<number | null>(
    location.state?.providerId ?? null,
  );
  ...
  useEffect(() => {
    async function loadAvailability() {
      ...
      console.log("[Agenda] availability data:", data);
  ...
  const selectedDaySlots = selectedAvailability?.slots ?? [];
  ...
  const handleConfirm = async () => {
    ...
    if (!selectedDate || !selectedTime || !providerId || !serviceId) {
        ...
        <div className={styles.card}>
          ...
          {selectedDate && selectedDaySlots.length > 0 && (
            <div className={styles.slotDropdown}>
              <p className={styles.slotTitle}>Horários disponíveis</p>
              <div className={styles.slotGrid}>
                {selectedDaySlots.map((slot) => (
                  <button
                    key={slot.time}
                    type="button"
                    className={`${styles.slotItem} ${!slot.available ? styles.slotUnavailable : ""} ${selectedTime === slot.time ? styles.slotSelected : ""}`}
